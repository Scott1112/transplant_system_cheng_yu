...
class MT24X(ReqResSerial):
    def __init__(self, acc_step, dec_step, vec_step, port, baudrate, ratio=1.8195,terminator = "\r\n"):
        ...
        # ----- 設定位置參數 -----
        # 吸取
        self.block_init_cam = [50000, -38000, -6000] #相機拍攝位置
        self.block_init_pos = [67000, -43000, -32500] #吸取初始位置
        self.z_safe_pos = -32500 #Z軸安全高度
        self.z_pick_pos = -40300 #Z軸吸取高度
        # 放置
        self.plate_size = [10, 10]
        self.plate_init_pos = [79800, -76000, -32500] #放的第一格
        self.z_place_pos = [-32500]
        self.plate_step = [3600, -3600] #[X, Y]有方向

        self.test_point = [57000, -60000, -40300]

        # transform matrix test point
        self.motor_point = []
    ...
    def move_to_center(self, point, frame_pos=False, absolute=False): #轉移矩陣
        ...
        motor_x = int(point[0]*x_ratio[0]+x_ratio[1])
        motor_y = int(point[1]*y_ratio[0]+y_ratio[1])
        motor_point = [motor_x, motor_y]

        self.move_MODE_P(0, motor_point[0])
        self.move_MODE_P(1, motor_point[1], wait=True)
